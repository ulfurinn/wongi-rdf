...
int check_opened_stream( pParserComplect complect ) {
...
VALUE turtle_parser_parse_string( int argc, VALUE * argv, VALUE self ) {
    VALUE source_string;
    VALUE working_document = Qnil;
    pParserComplect complect;
    ...
    if ( argc == 0 || ( argc > 0 && NIL_P(argv[0]) ) ) {
        rb_raise( rb_eArgError, "string required" );
    }
    ...
    Data_Get_Struct( self, ParserComplect, complect );
    string = (pANTLR3_UINT8) StringValueCStr( source_string );
    if( !open_string_stream(complect, string) ) {
        return working_document;
    }
    alloc = fill_parser_complect( complect );
...
void define_parser() {
    ...
    rb_define_method( cParser, "parse_string", &turtle_parser_parse_string, -1 );
}
